...
-- 各文字の最大連続登場回数のリストを作り、足し込む
solve :: String -> Int
solve xs =
  let substrs = foldStr xs 1
   in sum $ maximumSubStr substrs <$> ['a' .. 'z']

-- 指定された文字が最大何回登場するかを返す
maximumSubStr :: [SubStr] -> Char -> Int
maximumSubStr substrs c =
  let filtered = snd <$> filter (\a -> c == fst a) substrs
   in if null filtered then 0 else maximum filtered

-- 各文字を連続した登場回数に畳み込む
foldStr :: String -> Int -> [SubStr]
foldStr [x] n = [(x, n)]
foldStr (x : xs) n =
  if x == head xs
    then foldStr xs (n + 1)
    else (x, n) : foldStr xs 1

-- 同一文字の登場回数
type SubStr = (Char, Int)
