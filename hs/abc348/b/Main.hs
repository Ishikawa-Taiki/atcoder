...
main = do
  n <- getLineToInt
  xxs <- getContentsToIntTuples2
  printArrayWithLn $ solve xxs

solve :: [(Int, Int)] -> [Int]
solve xxs =
  let doublePoints = bimap int2Double int2Double <$> xxs
      withIndex = zip [1 ..] doublePoints
   in minimum . filterMaxDistance withIndex <$> doublePoints

filterMaxDistance :: [(Int, (Double, Double))] -> (Double, Double) -> [Int]
filterMaxDistance all target =
  let values = (\(idx, point) -> (idx, distanceTwoPoints point target)) <$> all :: [(Int, Double)]
      maxValue = maximum $ snd <$> values
   in fst <$> filter ((==) maxValue . snd) values
