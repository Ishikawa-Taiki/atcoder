...
main = do
  n <- getLineToInt
  xxs <- fmap ((\(a : b : _) -> (b, a)) . (fmap bsToInteger . BS.words)) . BS.lines <$> BS.getContents
  print $ solve xxs

solve :: [(Integer, Integer)] -> Integer
solve xxs =
  let minimums = M.fromListWith min xxs
      minimumValues = M.elems minimums
   in maximum minimumValues
