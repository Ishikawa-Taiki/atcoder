...
main = do
  ...
  ws <- getLineToIntegerArray
  print $ solve as ws n

solve :: [Integer] -> [Integer] -> Integer -> Integer
solve as ws n =
  let base = zip as ws
      result = foldl acc (0, M.empty) base
   in fst result

acc :: (Integer, Map Integer Integer) -> (Integer, Integer) -> (Integer, Map Integer Integer)
acc (resultCost, minMap) (i, cost) =
  let isExist = M.member i minMap
      (s : l : _) = sort [minMap M.! i, cost]
   in if not isExist
        then (resultCost, M.insert i cost minMap)
        else (resultCost + s, M.insert i l minMap)
