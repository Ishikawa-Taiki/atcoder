...
solve xs as n k = result
  where
    peoples :: Array Int (Double, Double)
    peoples = listArray @Array (1, n) $ concatMap ((: []) . bimap fromIntegral fromIntegral) xs
    lights = map (peoples !) as
    result = maximum . map (\i -> minimum . map (distanceTwoPoints (peoples ! i)) $ lights) $ [1 .. n]
