...
solve goal wall hammer
  | (goal * wall) < 0 = abs goal -- ゴールと壁が原点から違う方向にある
  | abs goal < abs wall = abs goal -- ゴールの方が壁より手前にある
  | (wall * hammer) < 0 = abs goal + (abs hammer * 2) -- 壁とハンマーが原点から違う方向にある
  | abs hammer < abs wall = abs goal -- ハンマーの方が壁より手前にある
  | otherwise = -1 -- 壁の向こうにハンマーがあるため、ゴールに辿り着けない
