...
-- axbのタイルを縦横にnセット並べたものを出力する(次のセットは白黒.#が反転する)
-- b個nセット並べた一行分のタイルセットを、縦にa個nセット用意する
solve :: Int -> Int -> Int -> [String]
solve n a b =
  let oddN = lineString False b n
      evenN = lineString True b n
   in concat $ (\v -> replicate a (if odd v then oddN else evenN)) <$> [1 .. n]

lineString :: Bool -> Int -> Int -> String
lineString False b n = concat $ (\v -> replicate b (toChar . even $ v)) <$> [1 .. n]
lineString True b n = concat $ (\v -> replicate b (toChar . odd $ v)) <$> [1 .. n]
