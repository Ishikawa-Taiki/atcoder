...
main :: IO ()
main = do
  (n, a) <- getLineToIntTuple2
  ts <- getLineToIntArray
  printArrayWithLn $ solve ts a

solve :: [Int] -> Int -> [Int]
solve ts a = fst $ foldl (acc a) ([], 0) ts
  where
    acc :: Int -> ([Int], Int) -> Int -> ([Int], Int)
    acc waitTime (result, beforeFinish) come =
      let nextResult = (+) waitTime $ max beforeFinish come
       in (result ++ [nextResult], nextResult)
