...
-- Stateモナドで描こうとしたが、部分更新が難しかった
-- STUArray, IOUArray あたりで書き直した方が良さそうなので中断
main :: IO ()
main = do
  n <- getLineToInt
  xs <- getLineToIntArray
  q <- getLineToInt
  qs <- getContentsToStringArray
  let _ = (`execState` xs) $ do
    forM_ qs $ \query -> do
      let 
        (cv:kv:xv:_) = words query
        c = read cv :: Int
        k = read kv :: Int
        x = read xv :: Int
      if c == 1
