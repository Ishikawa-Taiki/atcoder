...
main = do
  ...
  printArrayWithSpace $ solve k g m
  ...
  let (gResult, mResult) = flip fix ((0, 0), 5) \loop ((grass, mag), n) ->
        if n == 0
          then (grass, mag)
            ...
            if grass == g
              then loop ((0, mag), n - 1) -- グラスが満タンならグラスの水を捨てる
              else
                if mag == 0
                  then loop ((grass, m), n - 1) -- マグカップが空ならマグカップを満タンにする
                  else do
                    let fillGrass = g - grass
                        toGrass = if fillGrass < mag then g else grass + mag
                        toMag = if fillGrass < mag then mag - fillGrass else 0
                    loop ((toGrass, toMag), n - 1) -- マグカップからグラスへ水を移す処理
   in [gResult, mResult]
