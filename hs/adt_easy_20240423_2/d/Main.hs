...
main = do
  n <- getLineToInt
  xxs <- replicateM n getLine
  putStrLn . bool "incorrect" "correct" $ solve xxs

solve :: [[Char]] -> Bool
solve xxs = xxs == reverseMatrix xxs
  where
    reverseMatrix xxs = fmap reverseResult <$> transpose xxs

reverseResult :: Char -> Char
reverseResult 'W' = 'L'
reverseResult 'L' = 'W'
reverseResult other = other
