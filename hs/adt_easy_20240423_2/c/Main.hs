...
main = do
  ...
  cs <- getLineToIntArray
  rs <- getLineToIntArray
  let cards = zip3 [1 ..] cs rs
  print $ solve cards t

solve :: [(Int, Int, Int)] -> Int -> Int
solve all@(x : xs) t =
  let tFilterd = filter ((==) t . snd3) all
      hFilterd = filter ((==) (snd3 x) . snd3) all
   in if null tFilterd
        then fst3 . maximumBy checkOrder $ hFilterd
        else fst3 . maximumBy checkOrder $ tFilterd
  where
    checkOrder a b = compare (thd3 a) (thd3 b)
