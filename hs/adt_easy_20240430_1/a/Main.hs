...
main = do
  ...
  (yy, mm, dd) <- getLineToIntTuple3
  printArrayWithSpace $ solve yy mm dd m d

solve :: Int -> Int -> Int -> Int -> Int -> [Int]
solve yy mm dd m d =
  let isDayEnd = dd == d
      dResult = bool (dd + 1) 1 isDayEnd
      isMonthEnd = mm == m
      mResult =
        if isDayEnd
          then bool (mm + 1) 1 isMonthEnd
          else mm
      yResult = bool yy (yy + 1) (isMonthEnd && isDayEnd)
