...
-- 各ビットが1である確率を計算する関数
probOne :: Int -> Int -> Double
probOne n m = fromIntegral (popCount (n .&. m)) / fromIntegral n

-- 各ビットが1である確率の和を計算する関数
probSum :: Int -> Int -> Double
probSum n m = foldl' (\acc k -> acc + probOne k m) 0.0 [0 .. 59]

-- popcountの和を計算する関数
popCountSum :: Int -> Int -> Int
popCountSum n m = round (probSum n m * fromIntegral n) `mod` modulus
