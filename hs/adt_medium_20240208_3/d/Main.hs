...
-- 4つのマス上にコマをのせる+書かれている数値分ずらす操作を繰り返し、最終的に4つのコマ内から出ていったコマの数を求める
-- 最後から合算していった時にマスの数を超えたところから先全部溢れるので、長さの差を求める(超えた最初の要素を求めたら十分な気もする)
solve :: [Int] -> Int
solve xs =
  let op = debugProxy $ scanr1 (+) xs
   in length xs - length (filter (<= 3) op)
