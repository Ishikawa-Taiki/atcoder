...
-- ゴールに到達できる場合は最短距離を、到達できない場合は-1を出力する
-- 負の方向の座標が存在するので、方向を確認しながらパターンを分類する
solve :: Int -> Int -> Int -> Int
solve goal wall hammer =
  let needsWallBreak = debugProxy $ case (0 < goal, 0 < wall) of
        (True, True) -> wall < goal
        (False, False) -> wall > goal
        _ -> False
      canWallBreak = debugProxy $ case (0 < wall, 0 < hammer) of
        (True, True) -> hammer < wall
        (False, False) -> hammer > wall
        _ -> True
      hammerDistance =
        debugProxy $
          if (0 < goal && 0 < hammer) || (0 > goal && 0 > hammer)
            then 0
            else hammer * 2
   in if needsWallBreak
        then
          if canWallBreak
            then abs goal + abs hammerDistance
            else -1
        else abs goal
