...
solve xs n = result
  where
    ys = sortBy (flip compare) $ S.toList . S.fromList $ xs
    len = length ys
    a = listArray @UArray (1, len) ys
    m = countElements $ f <$> xs
    f x = fst $ binarySearch (\i -> a ! i > x) (0, succ len)
    result = g <$> [0 .. pred n]
    g k = fromMaybe 0 (m M.!? k)
