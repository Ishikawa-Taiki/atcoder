...
main = do
  ...
  xs <- replicateM m $ do
    (k : as) <- getLineToIntList
    return (k, as)
  bs <- getLineToIntList
  printListWithLn $ solve xs bs n m

solve :: [(Int, [Int])] -> [Int] -> Int -> Int -> [Int]
solve xs bs n m = result
  where
    b = listArray @UArray (1, n) $ map snd . sort $ zip bs [1 .. n]
    dishes = maximum . map (b !) . snd <$> xs -- その料理が食べられるようになる日
    e = countElements dishes -- その日に食べられるようになった料理の数
    result = reverse . snd . foldl f (0, []) $ [1 .. n]
    f (count, result) day = (new, new : result)
      where
        new = count + M.findWithDefault 0 day e -- その日までに食べられるように料理の数の合計
