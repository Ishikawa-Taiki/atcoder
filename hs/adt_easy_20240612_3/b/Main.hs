...
main = do
  ...
  printArrayWithSpace $ solve xs as

solve :: [Int] -> [Int] -> [Int]
solve (n : p : q : r : s : _) as =
  let start = debugProxy $ take (p - 1) as
      left = debugProxy $ take (q - (p - 1)) . drop (p - 1) $ as
      center = debugProxy $ take (r - q - 1) . drop q $ as
      right = debugProxy $ take (s - (r - 1)) . drop (r - 1) $ as
      end = debugProxy $ drop s as
   in start ++ right ++ center ++ left ++ end
