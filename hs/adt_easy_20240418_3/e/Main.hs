...
solve :: [Int] -> Int -> Bool
solve xs x = fst . foldl acc (False, S.fromList []) $ xs
  where
    acc (found, candidates) target =
      let newCandidates = appendCandidates candidates (calcCandidates x target)
       in ( found || S.member target newCandidates,
            newCandidates
          )

appendCandidates :: Set Int -> Set Int -> Set Int
appendCandidates = S.union

calcCandidates :: Int -> Int -> Set Int
calcCandidates x iORj =
  let useItoJ = (x - iORj) * (-1)
      useJtoI = (x + iORj)
   in fromList [useItoJ, useJtoI]
