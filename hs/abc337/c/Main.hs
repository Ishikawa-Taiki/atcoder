...
solve :: [Int] -> [Int]
solve xs = flip scanMap (-1) $ createOrderMap xs

-- 指定されたキーから後ろのキーリストを作成する
scanMap :: M.Map Int Int -> Int -> [Int]
scanMap orderMap target
  | M.member target orderMap =
    let currentIndex = orderMap M.! target
     in currentIndex : scanMap orderMap currentIndex
  | otherwise = []

-- キー1が順序情報、キー2が人N(index)
createOrderMap :: [Int] -> M.Map Int Int
createOrderMap xs = M.fromList $ zip xs [1 ..]
