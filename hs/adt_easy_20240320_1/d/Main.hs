...
main = do
  ...
  xy <- replicateM n getLineToIntTuple2
  print $ solve xy as

solve :: [(Int, Int)] -> [Int] -> Double
solve people withLight =
  let withoutLight = debugProxy $ filter (not . flip elem withLight) [1 .. length people]
      lightPeople = debugProxy $ (!!) people . subtract 1 <$> withLight
      noLightPeople = debugProxy $ (!!) people . subtract 1 <$> withoutLight
   in maximum $ (\no -> minimum $ distanceTwoPoints (bimap int2Double int2Double no) . bimap int2Double int2Double <$> lightPeople) <$> noLightPeople
