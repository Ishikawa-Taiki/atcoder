...
solve xs h w = result
  where
    ...
    vs = elem '#' <$> xs
    hs = elem '#' <$> transpose xs
    top = succ $ h - length (dropWhile not vs)
    bottom = length (dropWhileEnd not vs)
    left = succ $ w - length (dropWhile not hs)
    right = length (dropWhileEnd not hs)
    result = all f [a ! (i, j) | i <- [top .. bottom], j <- [left .. right]]
    f '#' = True
    f '?' = True
    f _ = False
