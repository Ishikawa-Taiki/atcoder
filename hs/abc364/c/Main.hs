...
main :: IO ()
main = do
  param@(n, x, y) <- arrayToTuple3 . bsToIntegerList <$> BS.getLine
  as <- getLineToIntegerArray
  bs <- getLineToIntegerArray
  print $ solve as bs param

type Pair = (Integer, Integer)

solve :: [Integer] -> [Integer] -> (Integer, Integer, Integer) -> Integer
solve as bs param@(n, x, y) =
  let sortA = debugProxy $ sortOn Down as
      sortB = debugProxy $ sortOn Down bs
      pairs = zip sortA sortB
   in snd . foldl acc ((0, 0), 0) $ pairs
  where
    acc :: (Pair, Integer) -> Pair -> (Pair, Integer)
    acc (p@(aSum, bSum), count) (currentA, currentB) =
      let found = aSum > x || bSum > y
       in if found
            then (p, count)
            else ((aSum + currentA, bSum + currentB), count + 1)
