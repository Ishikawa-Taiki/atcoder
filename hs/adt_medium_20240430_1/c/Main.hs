...
main = do
  n <- getLineToInt
  src <- replicateM n getLineToIntArray
  dst <- replicateM n getLineToIntArray
  let srcB = fmap (1 ==) <$> src
      dstB = fmap (1 ==) <$> dst
  printYesNo $ solve srcB dstB
...
solve src dst =
  let r0 = src
      ...
      r2 = rotate r1
      r3 = rotate r2
   in any (`implicationMatrix` dst) [r0, r1, r2, r3]

rotate :: [[a]] -> [[a]]
rotate = reverse . transpose

implicationMatrix :: [[Bool]] -> [[Bool]] -> Bool
implicationMatrix src dst = all lineCheck $ zip src dst
  where
    lineCheck (srcLine, dstLine) = all implication $ zip srcLine dstLine
    implication (srcItem, dstItem) = not srcItem || dstItem
