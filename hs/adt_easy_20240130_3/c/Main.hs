...
-- 元々の友達リスト、すでに秘密を知られた人リスト、今まさに知られようとしている友達indexを受け取り、知れ渡った人数を計算する
solve :: [Int] -> [Int] -> Int -> Int
solve xs result c =
  let v = debugProxy $ xs !! (c -1)
   in if v `elem` result || length xs == length result
        then length result + 1
        else solve xs (result ++ [v]) v
