...
-- 先頭の人が他の誰よりも大きな数値になるために必要な値は？
-- 元から最大値だと追加は不要だが、2人以上が最大値を持っているケースだと1増加する必要がある
solve :: [Int] -> Int
solve xs =
  let value = maximum xs - head xs
   in bool (value + 1) 0 (2 > length (filter (== maximum xs) xs) && value == 0)
