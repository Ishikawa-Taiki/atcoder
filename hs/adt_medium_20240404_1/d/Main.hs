...
main = do
  ...
  printArrayWithSpace $ execState solve xs

-- 作業中のコードを仮登録
solve :: State [Int] ()
solve = do
  v <- getValue
  let first = checkDiffOne v
  addItems 2 [5, 5, 5]

createItems :: Int -> Int -> [Int]
createItems l r =
  let base = if l < r then [l .. r] else reverse [r .. l]
   in tail . init $ base

addItems :: Int -> [Int] -> State [Int] ()
addItems index items = state $ \c -> ((), take (index + 1) c ++ items ++ drop (index + 1) c)

getValue :: State [Int] [Int]
getValue = state $ \c -> (c, c)

checkDiffOne :: [Int] -> Maybe Int
checkDiffOne xs =
  let idx = [0 .. length xs - 2]
   in findIndex (\i -> abs ((xs !! i) - (xs !! (i + 1))) /= 1) idx
