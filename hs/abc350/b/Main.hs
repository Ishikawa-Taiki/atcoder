...
solve :: [Int] -> Int -> Int
solve xs n =
  let deleteList = foldl (\candidates item -> if S.member item candidates then S.delete item candidates else S.insert item candidates) (S.fromList []) xs
   in n - length deleteList
