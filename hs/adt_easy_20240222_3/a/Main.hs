...
-- 父/母/本人と順番に残vのシャンプーを使用していった場合、初めてシャンプーが不足するのは誰か
solve :: [Int] -> Char
solve (v : a : b : c : _) = use (a, b, c) v 'F'

use :: (Int, Int, Int) -> Int -> Char -> Char
use num value tag
  | value >= useValue tag num = use num (value - useValue tag num) (next tag)
  | otherwise = tag

useValue :: Char -> (Int, Int, Int) -> Int
useValue 'F' (a, b, c) = a
useValue 'M' (a, b, c) = b
useValue 'T' (a, b, c) = c
