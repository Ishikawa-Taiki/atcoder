...
-- ちょうど3 つのレピュニットの和として表せる整数のうち n 番目に小さいもの
solve :: Int -> Int
solve n = debugProxy (sort . toList . fromList $ [repunit x + repunit y + repunit z | x <- [1 .. 12], y <- [1 .. 12], z <- [1 .. 12]]) !! (n -1)

-- いい感じに求めようとしたけど、桁が順に上がるわけでもなく難しかったので↑総当たり
-- solve :: Int -> Int
-- solve n =
--   let base = n - 1
--       d = base `div` 3
...
--       y = d + 1 + bool 0 1 (m >= 2)
--       z = d + 1 + bool 0 1 (m >= 1)
--    in debugProxy (repunit x + repunit y + repunit z)

-- n 番目に小さいレピュニット値を返す
repunit :: Int -> Int
repunit n = read $ intToDigit <$> (!!) (flip replicate 1 <$> [1 ..]) (n - 1)
