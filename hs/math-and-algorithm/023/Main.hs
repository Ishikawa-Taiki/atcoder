...
main = do
  n <- getLineToInt
  blue <- getLineToIntArray
  red <- getLineToIntArray
  print $ solve blue red

solve :: [Int] -> [Int] -> Double
solve blue red = dice blue + dice red

-- 目に書かれた数値のリストから期待値を返却する(等確率前提)
dice :: [Int] -> Double
dice xs = sum (fromIntegral <$> xs) / fromIntegral (length xs)
