...
solve n = dpTable
  where
    dpTable = listArray @Array (0, n) [calc i | i <- [0 .. n]]
    calc i
      | i <= 0 = 1
      | i == 1 = 1
      | otherwise = dpTable ! (i - 2) + dpTable ! (i - 1)

-- 1段or2段で進むことが出来る場合の、階段を登るルートのパターン数を算出する
--  2種類の遷移方法毎の結果を求めるので、n段目の遷移方法はn-2段目までの遷移方法とn-1段目までの遷移方法を足し合わせたものになる
--  最初の一歩を踏み出す部分は毎回違うルートのはずなので、そこで1数えるようにする
