...
-- N日目のA時間とB時間はバラバラのままだと扱いづらいので、転置行列でセットにして日毎の期待値を算出し、最後に畳み込む
solve :: [Int] -> [Int] -> Double
solve as bs = sum $ expectedValue <$> transpose [as, bs]
  where
    expectedValue (a : b : _) = (fromIntegral a * (2 / 6)) + (fromIntegral b * (4 / 6))
