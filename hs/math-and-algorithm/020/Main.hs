...
-- 候補と選択済みから要素を絞り込んでいく
-- すでに1000を超えるような組み合わせについては走査する必要がなさそうだが一旦置いておく
solve :: [Int] -> [Int] -> Int
solve [] selected = case length $ debugProxy selected of
  5 -> checkMatchCase selected
  _ -> 0
solve (x : xs) selected = case length $ debugProxy selected of
  5 -> checkMatchCase selected
  _ -> solve xs (x : selected) + solve xs selected

checkMatchCase :: [Int] -> Int
checkMatchCase selected = bool 0 1 $ sum selected == 1000

debugProxy :: [Int] -> [Int]
debugProxy selected =
  let !_ = debug "selected" selected
   in selected
