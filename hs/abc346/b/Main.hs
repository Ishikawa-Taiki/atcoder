...
solve :: Int -> Int -> Bool
solve w b =
  let testStr = take 400 $ cycle "wbwbwwbwbwbw"
      checkLen = w + b
   in not . null $ [1 | i <- [0 .. 399 - checkLen], let targetStr = take checkLen . drop i $ testStr, let wb = countWB targetStr, fst wb == w && snd wb == b]

-- wとbの数を返す
countWB :: String -> (Int, Int)
countWB str =
  let w = length . filter (== 'w') $ str
      b = length . filter (== 'b') $ str
   in (w, b)
