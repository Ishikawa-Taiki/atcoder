...
solve n = result
  where
    ...
    result = chunksOfList (succ end) [grid n i j | i <- [0 .. end], j <- [0 .. end]]
...
grid :: Int -> Int -> Int -> Char
grid 0 _ _ = '#'
grid l i j
  | isCenter l i j = '.'
  | otherwise = grid (pred l) (i `mod` 3) (j `mod` 3)

isCenter :: Int -> Int -> Int -> Bool
isCenter l i j = (b1 <= i && i < b2) && (b1 <= j && j < b2)
  where
    b1 = 3 ^ pred l
    b2 = 2 * b1
