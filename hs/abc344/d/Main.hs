...
main = do
  ...
  let candidates = (\(s : ss) -> ss) . words <$> xs
  print $ solve t (reverse candidates) 0

-- 時間切れで中断(末尾から一致見ていったらいけそう)
solve :: String -> [[String]] -> Int -> Int
solve t (candidate : candidates) sum =
  let !matchSuffix = debugProxy $ filter (`isSuffixOf` t) candidate
   in 1
