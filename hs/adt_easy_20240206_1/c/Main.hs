...
-- 大文字、小文字が一文字以上含まれていて、全ての文字が異なっているか
solve :: String -> Bool
solve s =
  let containsUpper = any isUpper s
      containsLower = any isLower s
      different = length s == length (toList . fromList $ s)
   in containsUpper && containsLower && different
