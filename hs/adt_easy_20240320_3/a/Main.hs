...
main :: IO ()
main = do
  (m, d) <- getLineToIntTuple2
  (year, month, day) <- getLineToIntTuple3
  printArrayWithSpace $ solve m d year month day

solve :: Int -> Int -> Int -> Int -> Int -> [Int]
solve m d year month day =
  let isLastDay = day == d
      isLastMonth = month == m
      resultD =
        if isLastDay
      ...
      resultY = if isLastMonth && isLastDay then year + 1 else year
   in [resultY, resultM, resultD]

-- solve :: Int -> Int -> Int -> Int -> Int -> [Int]
-- solve m d year month day =
--   let resultD = bool (day + 1) 1 (day == d)
--       plusM = bool month (month + 1) (day == d)
--       resultM = bool (plusM + 1) 1 (plusM == m)
--       resultY = bool year (year + 1) (plusM == m)
--    in [resultY, resultM, resultD]
