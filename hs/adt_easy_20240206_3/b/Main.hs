...
main = do
  x <- getLineToInt
  printYesNo $ solve x 0

-- 321-like Number(左から順に大きな数字になっている整数) かどうかを返却する
-- 評価中の数値と、前の桁を踏まえた結果としていくつより大きくなければならないかを受け、条件を満たしているかどうかを下の桁から計算していく
solve :: Int -> Int -> Bool
solve 0 _ = True
solve x n =
  let value = x `mod` 10
   in value >= n && solve (x `div` 10) (value + 1)
