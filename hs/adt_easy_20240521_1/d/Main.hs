...
main = do
  ...
  query <- getLineToIntArray
  let koma = listArray @UArray (1, k) as
      result = solve n koma query
  printArrayWithSpace $ map (result !) (range (bounds result))

solve :: Int -> UArray Int Int -> [Int] -> UArray Int Int
solve max koma [] = koma
solve max koma (q : query) =
  let komaRange = bounds koma
      targetPos = koma ! q
      nextPos = koma ! (q + 1)
   in if (targetPos == max) || ((q < snd komaRange) && (targetPos + 1) == nextPos)
        then solve max koma query
        else solve max (koma // [(q, targetPos + 1)]) query
