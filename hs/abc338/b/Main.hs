...
solve :: String -> Char
solve xs =
  let list = (\c -> (c, length (filter (== c) xs))) <$> "abcdefghijklmnopqrstuvwxyz"
   in fst . head $ sortBy (\a b -> if snd a == snd b then compare (fst a) (fst b) else compare (snd b) (snd a)) list
