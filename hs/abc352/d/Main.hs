...
-- solve xs k n =
--   let m = M.fromList $ zip xs ([1 ..] :: [Int])
--       sorted = listArray @UArray (1, n) $ sort xs
--    in foldl acc 9999999999 [k -1 .. n]
--   where
--     acc minValue i =
--       let diff = (m M.! (sorted ! i)) - (m M.! (sorted ! i - k + 1))
--        in min minValue diff

solve2 :: Int -> Int -> [Int] -> Int
solve2 n k p = minimum $ map (\i -> (map !! i) - (map !! (i - k + 1)) + 1) [k -1 .. n -1]
  where
    map = sortOn snd $ zip p [1 .. n]
