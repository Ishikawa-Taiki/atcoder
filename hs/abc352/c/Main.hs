...
main = do
  n <- getLineToInt
  xs <- fmap (arrayToTuple2 . bsToIntegerList) . BS.lines <$> BS.getContents
  print $ solve xs

solve :: [(Integer, Integer)] -> Integer
solve xs =
  let base = foldl acc (0, 0) xs
   in uncurry (+) base
  where
    acc (shoulderSum, maxHead) (shoulder, top) =
      let headValue = top - shoulder
       in (shoulderSum + shoulder, max headValue maxHead)
