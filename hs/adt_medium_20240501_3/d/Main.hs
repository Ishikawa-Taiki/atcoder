...
data Direction = N | S | W | E deriving (Show, Enum)

type Position = (Direction, (Int, Int))

calc :: String -> (Direction, (Int, Int))
calc s = (`execState` (E, (0, 0))) $ do
  forM_ s $ \c ->
    step c

step :: Char -> State Position ()
step 'S' = state $ \(d, (x, y)) ->
  let (f1, f2) = move d
   in ((), (d, (f1 x, f2 y)))
step 'R' = state $ \(d, p) -> ((), (r90 d, p))

r90 :: Direction -> Direction
r90 N = E
r90 E = S
r90 S = W
r90 W = N
...
move N = (id, (+ 1))
move E = ((+ 1), id)
move S = (id, subtract 1)
move W = (subtract 1, id)
