...
main :: IO ()
main = do
  (n, s, k) <- getLineToIntTuple3
  pq <- replicateM n getLineToIntTuple2
  print $ solve pq s k

-- 各注文毎の商品額p*注文個数qを畳み込み、合計額がs円を超えていなければ送料k円追加
solve :: [(Int, Int)] -> Int -> Int -> Int
solve pq s k =
  let total = sum $ uncurry (*) <$> pq
   in if s <= total then total else total + k
