...
echo "--- Creating submission.cabal ---"
cat > submission.cabal <<'CABAL_EOF'
cabal-version: 3.4
name: submission
version: 0.1.0.0
build-type: Simple
...
echo "--- Creating cabal.project ---"
cat > cabal.project <<'PROJECT_EOF'
packages: ./submission.cabal
optimization: 2
package *
  compiler: ghc
  ghc-options: -fllvm -Wall
allow-newer: witherable:containers,semialign:containers,scientific:containers,parallel:containers,megaparsec:containers,foldl:containers,linear-generics:containers,indexed-traversable:containers,hashable:containers
PROJECT_EOF
